...
function initialize() {
  ...
  loginUserForm = gradioApp().querySelector(
    ".gradio-container > .main > .wrap > .panel > .form"
  );
  ...
  chatbotWrap = gradioApp().querySelector(
  ...
  if (
    (longTimeNoCheck && !updateInfoGotten && !isLatestVersion) ||
    (isLatestVersion && !updateInfoGotten)
  ) {
    updateLatestVersion();
  }
  return true;
...
function showConfirmationDialog(a, file, c) {
  if (file != "") {
    var result = confirm(
      i18n(deleteConfirm_i18n_pref) + file + i18n(deleteConfirm_i18n_suff)
    );
    if (result) {
      return [a, file, c];
    }
  ...
  if (user_input_ta) {
    ...
    user_input_ta.addEventListener("keydown", function (event) {
      ...
      if (event.code === "ArrowUp" || event.code === "ArrowDown") {
        ...
        if (value && key_down_history.indexOf(value) === -1) return;
        // 对于需要响应的动作，阻止默认行为。
...
function setChatbotHeight() {
  ...
  if (isInIframe) {
    ...
    if (screenWidth <= 320) {
      chatbot.style.height = `calc(var(--vh, 1vh) * 100 - ${
        statusDisplayHeight + 150
      }px)`;
      chatbotWrap.style.maxHeight = `calc(var(--vh, 1vh) * 100 - ${
        statusDisplayHeight + 150
      }px - var(--line-sm) * 1rem - 2 * var(--block-label-margin))`;
    } else if (screenWidth <= 499) {
      chatbot.style.height = `calc(var(--vh, 1vh) * 100 - ${
        statusDisplayHeight + 100
      }px)`;
      chatbotWrap.style.maxHeight = `calc(var(--vh, 1vh) * 100 - ${
        statusDisplayHeight + 100
      }px - var(--line-sm) * 1rem - 2 * var(--block-label-margin))`;
  ...
  for (var i = 0; i < attempt; i++) {
    setTimeout(
      () => {
        ...
        gradioApp()
          .querySelectorAll("#chuanhu-chatbot .message-wrap .message.bot")
...
window
  ...
  .addEventListener("change", adjustDarkMode);
...
// button svg code
const copyIcon =
  '<span><svg stroke="currentColor" fill="none" stroke-width="2" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round" height=".8em" width=".8em" xmlns="http://www.w3.org/2000/svg"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg></span>';
const copiedIcon =
  '<span><svg stroke="currentColor" fill="none" stroke-width="2" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round" height=".8em" width=".8em" xmlns="http://www.w3.org/2000/svg"><polyline points="20 6 9 17 4 12"></polyline></svg></span>';
const mdIcon =
